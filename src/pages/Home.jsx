...
export default function Home() {
  ...
  const filtered = useMemo(() => {
    let list = [...resources];
    if (filterSub  !== "All") list = list.filter(r => r.courseCode === filterSub);
    ...
    if (sort === "mine")   list = list.filter(r => r.uploadedBy === user?.uid);
    return list;
  }, [resources, filterSub, filterType, searchQ, sort, user]);

  const countByCode = useMemo(() => {
    const m = {};
    resources.forEach(r => { m[r.courseCode] = (m[r.courseCode] || 0) + 1; });
    return m;
  }, [resources]);
  ...
  const savedCount = resources.filter(r => r.savedBy?.includes(user?.uid)).length;
